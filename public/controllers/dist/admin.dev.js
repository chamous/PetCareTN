...
admin.controller('adminController', ['$rootScope', '$anchorScroll', '$location', '$http', '$scope', 'shareDataService', function ($rootScope, $anchorScroll, $location, $http, $scope, shareDataService) {
  $scope.users = [];
  $scope.petPostings = [];
...
admin.controller('adminModalController', ['$rootScope', '$http', '$scope', 'shareDataService', '$cookies', function ($rootScope, $http, $scope, shareDataService, $cookies) {
  // Get user Id from shared data
  // Make http request to ban the user
  $scope.ConfirmBan = function () {
    var userID = shareDataService.getData(); // Creare object to be sent through the put request 

    var dataObj = {
      id: userID
    }; // Make an http put request since Angular doesn't provide update

    $http.put('/api/users/' + userID + '/ban', {
      data: dataObj
  ...
  $scope.ConfirmDelete = function () {
    ...
    var postType = shareDataService.getData().postingType; // If type is sitter_posting, make an API call to delete /api/sitterpostings/:id

    if (postType === 'sitter_posting') {
      // Make an http delete request to delete post with a given id
      $http["delete"]('/api/sitterpostings/' + postID).success(function (data, status, headers, config) {
    ...
    if (postType === 'pet_posting') {
      // Make an http delete request to delete post with a given id
      $http["delete"]('/api/petpostings/' + postID).success(function (data, status, headers, config) {
        // If delete request was successful, update the view (i.e hide post)
  ...
  $scope.ConfirmAdmin = function () {
    ...
    var userID = shareDataService.getData().userID; // Create object to be sent through the put request 
    ...
    $http.put('/api/users/' + userID + '/role', {
      data: dataObj
    }).success(function (data, status, headers, config) {
      var newText;
  ...
  $scope.sendMsg = function () {
    ...
    var from = $cookies.get('userID'); // Cteate object to be sent through the post request
  ...
  $scope.editUserInfo = function (isValid) {
    ...
    if (isValid) {
      var userID = shareDataService.getData(); // Create object to be sent through the POST request
  ...
  $scope.editPostingInfo = function (isValid) {
    ...
    if (isValid) {
      ...
      var postID = shareDataService.getData().postingID; // Create object to be sent through the POST request
